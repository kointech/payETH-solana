...
/// Bridge PAYE tokens from Solana to another chain.
///
/// For the Native OFT type (Solana remote chain):
///   - Protocol fee is transferred to the escrow token account.
///   - Remaining tokens are burned.
///   - A LayerZero message is sent to the destination chain's endpoint.
///   - The destination chain mints an equivalent amount to the recipient.
#[event_cpi]
#[derive(Accounts)]
...
pub struct Send<'info> {
    ...
    /// Source token account owned by `signer`.
    #[account(
        mut,
...
impl Send<'_> {
    pub fn apply(
        ...
        // Refill the inbound rate limiter (tokens leaving means inbound capacity frees up)
        if let Some(rl) = ctx.accounts.peer.inbound_rate_limiter.as_mut() {
            rl.refill(amount_received_ld)?;
        }

        // Native OFT: burn tokens, transfer fee portion to escrow
...
pub struct SendParams {
    pub dst_eid: u32,
    /// Recipient address on the destination chain, encoded as bytes32.
    pub to: [u8; 32],
    pub amount_ld: u64,
