...
/// Full fee / amount breakdown quote for a send operation.
#[derive(Accounts)]
...
impl QuoteOFT<'_> {
    pub fn apply(ctx: &Context<QuoteOFT>, params: &QuoteOFTParams) -> Result<QuoteOFTResult> {
        ...
        let oft_limits = OFTLimits { min_amount_ld: 0, max_amount_ld: u64::MAX };

        let mut fee_details = if amount_received_ld + oft_fee_ld < amount_sent_ld {
            vec![OFTFeeDetail {
        ...
        Ok(QuoteOFTResult {
            oft_limits,
            oft_fee_details: fee_details,
            oft_receipt: OFTReceipt { amount_sent_ld, amount_received_ld },
        })
    }
}
