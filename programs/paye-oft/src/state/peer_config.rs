...
/// Per-peer configuration stored in a PDA.
///
/// PDA seeds: ["Peer", oft_store.key(), remote_eid.to_be_bytes()]
#[account]
#[derive(InitSpace)]
pub struct PeerConfig {
    /// Remote OFT contract address encoded as bytes32.
    pub peer_address: [u8; 32],
    /// Minimum message options enforced for sends to this peer.
    pub enforced_options: EnforcedOptions,
    /// Optional outbound rate limiter (tokens per second leaving Solana).
    pub outbound_rate_limiter: Option<RateLimiter>,
    /// Optional inbound rate limiter (tokens per second arriving on Solana).
    pub inbound_rate_limiter: Option<RateLimiter>,
    /// Per-peer protocol fee override (basis points).  None = use store default.
...
impl RateLimiter {
    ...
    pub fn refill(&mut self, extra_tokens: u64) -> Result<()> {
        ...
        if current_time > self.last_refill_time {
            ...
            new_tokens =
                new_tokens.saturating_add(elapsed.saturating_mul(self.refill_per_second));
        }
...
impl EnforcedOptions {
    pub fn get_enforced_options(&self, composed_msg: &Option<Vec<u8>>) -> Vec<u8> {
        if composed_msg.is_none() { self.send.clone() } else { self.send_and_call.clone() }
    }
    ...
    pub fn combine_options(
        ...
        let enforced = self.get_enforced_options(compose_msg);
